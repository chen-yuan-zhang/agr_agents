...
def create_root_label(pos_state, target, heuristic, fly_altitude=None):
    next_g = 0
    next_h = heuristic(pos_state[0], target, fly_altitude)
    next_f = next_g + next_h
    next = Label(pos_state, 0, next_g, next_f, None, None)
    return next

def extend(current, pos_state, target, cost, heuristic, action, fly_altitude=None):
